...
| Reasoning key follows a changed custom endpoint | Preset origins are fixed; custom vault entries are keyed to the complete normalized base URL; environment keys apply only to environment-defined configuration | A user can explicitly enter a credential for a reviewed custom HTTPS endpoint |
...
| Ambient microphone disclosure | Tap-to-talk is the default and STT is disabled until configured; cloud wake mode carries an explicit upload/usage warning; transcription uses fixed provider origins and bounded audio | Cloud wake sends each detected utterance to the configured STT provider and can incur usage charges |
| Voice provider substitution | Rust owns fixed OpenAI and ElevenLabs origins, disables redirects, bounds payloads, and retrieves credentials from Windows Credential Manager or explicit environment variables | The selected speech provider receives audio or response text intentionally sent to it |
